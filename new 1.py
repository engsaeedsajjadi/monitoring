...
# ==============================================================================
# ایجاد کاربر پیش‌فرض supervisor اگر وجود ندارد
# ==============================================================================

def create_default_supervisor():
    """ایجاد کاربر supervisor پیش‌فرض برای اولین اجرای برنامه."""
    # 🔥 بهبود امنیتی: رمز عبور از متغیر محیطی خوانده می‌شود تا از هاردکد کردن جلوگیری شود.
    DEFAULT_ADMIN_USER = os.getenv("DEFAULT_ADMIN_USER", "admin")
    DEFAULT_ADMIN_PASSWORD = os.getenv("DEFAULT_ADMIN_PASSWORD", "admin123")
    
    try:
        with SessionLocal() as db:
            # بررسی وجود کاربر با نقش supervisor
            ...
            if not existing_supervisor:
                ...
                print("   !! مهم: این رمز عبور را در اولین فرصت تغییر دهید و !!")
                print("   !! در محیط تولید از متغیر محیطی DEFAULT_ADMIN_PASSWORD استفاده کنید. !!")
                print("=" * 60)
            else:
...
# فراخوانی تابع برای ایجاد کاربر پیش‌فرض در هنگام راه‌اندازی
...
def get_kpi_summary(db: Session = Depends(get_db)):
    ...
    for p in all_projects_with_steps:
        ...
        timeline = [(None, p.created_at)] + [(s.name_key, s.timestamp) for s in sorted_steps]

        for i in range(len(timeline) - 1):
            start_key, start_time = timeline[i]
            end_key, end_time = timeline[i+1]
            
            step_key_for_duration = start_key or StepNameKey.START_ASSEMBLY
            duration_hours = (end_time - start_time).total_seconds() / 3600
            
            if duration_hours >= 0:
                 durations[step_key_for_duration].append(duration_hours)
    ...
    ) if avg_step_durations else None
    bottleneck_step_name = STEP_KEY_TO_NAME_MAP.get(bottleneck_step_key, None)
...
if __name__ == "__main__":
    ...
    # ✅ بهبود: تنظیمات uvicorn بر اساس متغیر محیطی برای تفکیک توسعه و تولید.
    is_production = os.getenv("ENVIRONMENT") == "production"
    
    logger.info(f"Starting server in {'PRODUCTION' if is_production else 'DEVELOPMENT'} mode.")
    
    uvicorn.run(
        # ✅ بهبود: استفاده از نام فایل و اپلیکیشن برای اجرای استاندارد
        "__main__:app", 
        host="0.0.0.0",
        port=8000,
